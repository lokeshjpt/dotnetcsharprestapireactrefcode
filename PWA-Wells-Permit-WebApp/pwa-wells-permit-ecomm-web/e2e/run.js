#!/usr/bin/env node
// Interactive launcher for the ecomm regression suite.
//
//   npm run test:regression
//
// Prompts for:
//   1. Environment — local | dev | test | uat   (production is refused)
//   2. Applicant email id — used to seed the Apply flow / identify the application under test
...
// or the --env=<env> / --email=<addr> / --headed / --headless flags. Any other CLI args are
// forwarded to `playwright test` (e.g. `npm run test:regression -- regression.spec.js`).
...
async function main() {
  ...
  try {
    ...
    // Live mode: drive the REAL local API (no /api mocks). Requires the Entra bearer captured by the
    // intra auth.setup (run the intra live suite first, or sign in once) for the guarded write calls.
    const live = flags.live || /^(1|y|yes|true|on)$/i.test(String(process.env.PWA_LIVE || '')) || env !== 'local';

    console.log(`\n▶ Running ecomm regression against "${env}" as applicant <${email}>` +
