...
function HelpPage() {
  ...
  // The intra content pane (.staff-content) is the scroll container, not the window, so scroll
  // to its top — window.scrollTo would be a no-op here (and window.scrollY stays 0).
  const scrollToTop = useCallback((behavior = 'smooth') => {
    const scroller = scrollerRef.current || rootRef.current?.closest('.staff-content') || window;
    scroller.scrollTo({ top: 0, behavior });
  }, []);

  // Show a floating "back to top" button once the reader has scrolled past the hero. The help
  // content is long (many sections/screenshots), so a quick way back to the search box is handy.
  // Watch the actual scroll container's scrollTop (the content pane), falling back to the window.
  useEffect(() => {
    const scroller = rootRef.current?.closest('.staff-content') || window;
    scrollerRef.current = scroller === window ? null : scroller;
    const readTop = () => (scroller === window ? window.scrollY : scroller.scrollTop);
    const onScroll = () => setShowTop(readTop() > 600);
    onScroll();
  ...
  // Deep-link support: scroll to the section named in the URL hash (e.g. /help#search).
  useEffect(() => {
  ...
  }, [location.hash, alignToSection, scrollToTop]);
  ...
  return (
    <div className="page-shell help-page" ref={rootRef}>
      <div className="panel help-hero">
        <p className="help-hero__eyebrow">Help &amp; How-To</p>
        <h1 className="page-title">Using the permit processing portal</h1>
        <p className="page-subtitle">
          Step-by-step instructions, with screenshots, for the work queues, application review and
          approval, search, inspections, and reports. Search below and pick a topic to jump straight
          to it.
        </p>
      ...
      <div className="help-content">
        ...
        <p className="help-foot">
          Still need help? Contact the PWA Wells Permit support team or your ITD administrator.
        </p>
      </div>
