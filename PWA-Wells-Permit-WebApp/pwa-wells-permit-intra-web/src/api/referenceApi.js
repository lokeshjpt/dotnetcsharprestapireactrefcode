...
export async function getWorkTypes(cat) {
  const { data } = await axiosInstance.get('/api/ref/work-types', { params: cat ? { cat } : undefined });
  return data;
}

export async function getWellUseTypes(cat, type) {
  const { data } = await axiosInstance.get('/api/ref/well-use-types', { params: { cat, type } });
  return data;
}
...
export async function getInspectors() {
  const { data } = await axiosInstance.get('/api/ref/inspectors');
  return data;
}
